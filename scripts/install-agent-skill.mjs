...
function parseArguments(argv) {
  ...
  if (!["codex", "claude", "both", "portable"].includes(parsed.target)) throw new Error("--target must be codex, claude, both, or portable");
...
async function main() {
  ...
  for (const destination of destinations(parsed)) {
    if (await exists(destination)) {
      if (await sameSkill(source, destination)) {
    ...
    if (!parsed.dryRun) await cp(source, destination, { recursive: true, force: true });
