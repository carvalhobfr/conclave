...
const TOOLS: readonly ToolDefinition[] = [
  ...
  { name: "conclave_validate", description: "Independently validate a working, staged, branch, or commit change against an objective and optional structured claims. The returned verdict must not be overridden by agent confidence.", inputSchema: { type: "object", additionalProperties: false, required: ["objective"], properties: { objective: { type: "string", minLength: 1, maxLength: 2_000 }, source: { enum: ["working", "staged", "branch", "commit"] }, ref: { type: "string", maxLength: 200 }, contract: { type: "object" } } } },
